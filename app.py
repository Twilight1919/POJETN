...
csv_file_path = "https://github.com/nevermind78/DATALM/raw/main/1LM.csv" #les notes des tous les étudiants
# Chargement du fichier CSV en nettoyant les espaces dans la colonne Email
df = pd.read_csv(csv_file_path, delimiter=";", converters={"Email": lambda x: x.strip()}) # strip enleve les éspaces des  deuxcotés
df = df[df['GROUP']=='1LM3']
# Titre de l'application
st.title("NOTES  DS PYTHON")
st.header("1LM3 A.U 2023-2024") #sous titre
...
df["Note"] = pd.to_numeric(df["Note"], errors='coerce').fillna(0) # fillna converte les alph(A) en 0
